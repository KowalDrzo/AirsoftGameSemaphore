...
void dispBlink(uint8_t bright, _Bool ifRed) {
	tm1637SetBriOne(0, ifRed);
	HAL_Delay(100);
	tm1637SetBriOne(bright, ifRed);
	HAL_Delay(250);
}
...
void tm1637DisplayTab(unsigned char* digitArr, _Bool displaySeparator, _Bool ifRed) {

	unsigned char digits[4];

	for (int i = 0; i < 4; ++i) {

		digits[i] = segmentMap[digitArr[i]];
		if (i == 2 && displaySeparator) digits[i] |= 1 << 7;
	}
	...
	for (int i = 0; i < 4; ++i) {
		_tm1637WriteByte(digits[3 - i], ifRed);
	}
...
void _tm1637ClkHigh(_Bool ifRed) {
	...
	if(ifRed) HAL_GPIO_WritePin(RED_CLK_GPIO_Port, RED_CLK_Pin, 1);
	else HAL_GPIO_WritePin(BLUE_CLK_GPIO_Port, BLUE_CLK_Pin, 1);
}

void _tm1637ClkLow(_Bool ifRed) {

	if(ifRed) HAL_GPIO_WritePin(RED_CLK_GPIO_Port, RED_CLK_Pin, 0);
	else HAL_GPIO_WritePin(BLUE_CLK_GPIO_Port, BLUE_CLK_Pin, 0);
}

void _tm1637DioHigh(_Bool ifRed) {

	if(ifRed) HAL_GPIO_WritePin(RED_DIO_GPIO_Port, RED_DIO_Pin, 1);
	else HAL_GPIO_WritePin(BLUE_DIO_GPIO_Port, BLUE_DIO_Pin, 1);
}

void _tm1637DioLow(_Bool ifRed) {

	if(ifRed) HAL_GPIO_WritePin(RED_DIO_GPIO_Port, RED_DIO_Pin, 0);
	else HAL_GPIO_WritePin(BLUE_DIO_GPIO_Port, BLUE_DIO_Pin, 0);
}
