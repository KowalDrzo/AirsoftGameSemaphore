...
void _tm1637ClkHigh(_Bool ifRed) {
	...
	if(ifRed) HAL_GPIO_WritePin(CZER_CLK_GPIO_Port, CZER_CLK_Pin, 1);
	else HAL_GPIO_WritePin(NIEB_CLK_GPIO_Port, NIEB_CLK_Pin, 1);
}

void _tm1637ClkLow(_Bool ifRed) {

	if(ifRed) HAL_GPIO_WritePin(CZER_CLK_GPIO_Port, CZER_CLK_Pin, 0);
	else HAL_GPIO_WritePin(NIEB_CLK_GPIO_Port, NIEB_CLK_Pin, 0);
}

void _tm1637DioHigh(_Bool ifRed) {

	if(ifRed) HAL_GPIO_WritePin(CZER_DIO_GPIO_Port, CZER_DIO_Pin, 1);
	else HAL_GPIO_WritePin(NIEB_DIO_GPIO_Port, NIEB_DIO_Pin, 1);
}

void _tm1637DioLow(_Bool ifRed) {

	if(ifRed) HAL_GPIO_WritePin(CZER_DIO_GPIO_Port, CZER_DIO_Pin, 0);
	else HAL_GPIO_WritePin(NIEB_DIO_GPIO_Port, NIEB_DIO_Pin, 0);
}
