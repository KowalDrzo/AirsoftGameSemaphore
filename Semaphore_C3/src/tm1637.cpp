...
void dispBlink(_Bool ifRed) {
	...
	delay(100);
	tm1637SetBriOne(/*asgClock.bright*/ 3, ifRed);
	delay(250);
}
...
void _tm1637ClkHigh(_Bool ifRed) {
	...
	if(ifRed) digitalWrite(RED_CLK_PIN, 1);
	else digitalWrite(BLU_CLK_PIN, 1);
}

void _tm1637ClkLow(_Bool ifRed) {

	if(ifRed) digitalWrite(RED_CLK_PIN, 0);
	else digitalWrite(BLU_CLK_PIN, 0);
}
